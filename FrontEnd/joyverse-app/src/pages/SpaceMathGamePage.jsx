...
import { Star, Rocket, Sparkles, Trophy, ArrowRight, ArrowLeft } from 'lucide-react';
...
const SpaceMathGamePage = () => {
  ...
  // Set body overflow to hidden when component mounts and reset styles
  useEffect(() => {
    ...
    // Also ensure the root element doesn't constrain us
  ...
  // Start emotion detection when game starts
  useEffect(() => {
    if (gameStarted) {
      console.log('🎮 Starting emotion detection for Space Math Game');
      emotionDetectionService.startEmotionDetection();
    }
    
    return () => {
      console.log('🛑 Stopping emotion detection - Space Math Game unmounting');
  ...
  if (!gameStarted) {
    return (
      <div 
        className={`game-container ${currentTheme.bg}`}
      ...
      >
        ...
        <div className="welcome-screen-centered">
          ...
          <button
            onClick={goBack}
            className={`back-button-centered ${currentTheme.text}`}
          >
  ...
  return (
    ...
    >
      ...
      {stars.map(star => (
        ...
        <button
          onClick={goBack}
          className={`back-button-game ${currentTheme.text}`}
        >
          <ArrowLeft size={20} style={{ marginRight: 8 }} />
          Back to Dashboard
        </button>
